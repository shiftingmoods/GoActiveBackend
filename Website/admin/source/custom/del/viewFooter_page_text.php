...
<div id="body">	
		...
		<div id="firstColumn" >
		<div >
			<input id="add_item_buttom" type="button" style="width:<?php echo(strlen($tables)*9); ?>px" onClick="window.location='<?php echo 'edit'.$Table; ?>.php?id=<?php echo $selectedItemId; ?>'" value="<?php echo 'Edit '.$index->toView($Table); ?>" >
		</div>
		<table >
		</table>
		</div>
...
if(isset($selectedItemId))
			...
			<?php	
											...
											case 'text':
			?>
												<textarea disabled name="<?php echo $table; ?>[<?php echo $id; ?>]" id="<?php echo $Table.'-'.$outer.$optional; ?>" maxlength="<?php echo $type['length']; ?>" class="ckeditor" cols="80"  rows="10" ><?php echo $itm[$id]; ?></textarea>
			<?php	
											break;
											default:
			?>
